...
- New scale arrows start near the paper's top-left corner, with room above for the label. After resizing the grid or moving the arrows, use **Reset scale marker position** in Grid Scale to return existing arrows and their labels to that corner. This preserves the scale, font sizes, and deleted arrows, and supports undo.
