...
[Download Ray Studio for Windows x64](https://github.com/kw39/ray-studio/releases/download/v1.3.0/Ray-Studio-1.3.0-windows-x64.zip) · [Release details](https://github.com/kw39/ray-studio/releases/tag/v1.3.0)
...
- New scale arrows meet at the paper's exact top-left corner: the horizontal arrow follows the top border with its label above the grid, and the vertical arrow follows the left border with its label inside the grid. After resizing the grid or moving the arrows, use **Reset scale marker position** in Grid Scale to return existing arrows and their labels to that corner. This preserves the scale, font sizes, and deleted arrows, and supports undo. Fit page and exports include space for labels outside the grid.
